   ...
   public class PublisherDbContext : DbContext
    {
        ...
        protected override void OnModelCreating(ModelBuilder modelBuilder)
        {
            if (modelBuilder != null)
            {
                ...
                //    );
                modelBuilder.Entity<Book>()
                    .HasOne(m => m.BookPublisher)
                    .WithMany(a => a.Books)
                    .HasForeignKey(k => k.BookPublisherId);
                // .UsingEntity(ma => ma.ToTable("BookAuthor"));
                modelBuilder.Entity<Author>()
                  .HasOne(m => m.AuthorPublisher)
                  .WithMany(j => j.Authors)
                  .HasForeignKey(k => k.AuthorPublisherID);

                modelBuilder.Entity<Publisher>()
                  .HasMany(m => m.Books)
                  .WithOne(j => j.BookPublisher)
                  .HasForeignKey(k => k.BookPublisherId);

                modelBuilder.Entity<Publisher>()
                   .HasMany(m => m.Authors)
                   .WithOne(j => j.AuthorPublisher)
                   .HasForeignKey(k => k.AuthorPublisherID);
